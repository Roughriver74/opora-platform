	...
	try {
		...
		try {
			...
			if (!bitrix24Service.isEnabled()) {
				console.log(`[SUBMIT_FORM] ⏭️ Пропуск синхронизации - Bitrix24 отключен`)
				return
			}
					...
					for (const field of form.fields) {
						...
						if (
							...
							if (field.dynamicSource?.enabled) {
								...
								// Проверяем, включена ли интеграция с Bitrix24
								if (!bitrix24Service.isEnabled()) {
									console.log(`[GET_SUBMISSION] ⏭️ Пропуск обогащения данных - Bitrix24 отключен`)
									// Используем исходное значение без обогащения
									enrichedFormData[field.name] = value
									continue
								}
...
export const updateSubmissionStatus = async (req: Request, res: Response) => {
	try {
		...
		if (submission.bitrixDealId) {
			// Проверка, включена ли интеграция с Bitrix24
			if (!bitrix24Service.isEnabled()) {
				// Bitrix24 отключен, синхронизация не требуется
				return res.json({
					success: true,
					message: "Статус обновлен (Bitrix24 отключен)",
					data: submission,
				})
			}
...
export const updateSubmission = async (req: Request, res: Response) => {
	try {
		...
		try {
			...
			// Проверка, включена ли интеграция с Bitrix24
			if (!bitrix24Service.isEnabled()) {
				console.log(`[UPDATE_SUBMISSION] ⏭️ Пропуск синхронизации - Bitrix24 отключен`)
				// Данные уже сохранены в БД, синхронизация не требуется
			} else {
...
export const cancelSubmission = async (req: Request, res: Response) => {
	try {
		...
		if (submission.bitrixDealId) {
			// Проверка, включена ли интеграция с Bitrix24
			if (!bitrix24Service.isEnabled()) {
				// Bitrix24 отключен, локальная отмена выполнена
				return res.json({
					success: true,
					message: "Заявка отменена (Bitrix24 отключен)",
					data: submission,
				})
			}
