 ...
 */
export const syncWithBitrix = async (req: Request, res: Response): Promise<void> => {
  try {
    const { forceSync = false } = req.body;
    
    // Получаем всех пользователей из Битрикс24
    const bitrixUsers = await bitrix24Service.getUsers();
      ...
      try {
        ...
        if (user) {
          ...
          if (forceSync || !user.bitrix_id) {
            ...
            user.firstName = bitrixUser.NAME || bitrixUser.firstName || user.firstName;
            user.lastName = bitrixUser.LAST_NAME || bitrixUser.lastName || user.lastName;
