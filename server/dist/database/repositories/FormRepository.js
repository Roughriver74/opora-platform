...
class FormRepository extends BaseRepository_1.BaseRepository {
    constructor() {
        super(Form_entity_1.Form, 'form');
    ...
    async createWithFields(formData, fields) {
    ...
    async updateWithFields(formId, formData, fields) {
    ...
    async getFormStatistics(formId) {
        const stats = await this.createQueryBuilder('form')
            ...
            .addSelect('COUNT(DISTINCT CASE WHEN DATE(submission.createdAt) = CURRENT_DATE THEN submission.id END)', 'todaySubmissions')
            .addSelect('COUNT(DISTINCT field.id)', 'fieldCount')
            .addSelect('COUNT(DISTINCT CASE WHEN field.required = true THEN field.id END)', 'requiredFieldCount')
            .groupBy('form.id')
            .getRawOne();
        if (!stats) {
            return {
    ...
    async duplicateForm(formId, newName) {
        ...
        if (originalForm.fields && originalForm.fields.length > 0) {
            newForm.fields = originalForm.fields.map(field => {
                const newField = new FormField_entity_1.FormField();
