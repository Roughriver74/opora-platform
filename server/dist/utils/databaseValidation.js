 ...
 */
async function validateFormFieldsIntegrity() {
    const issues = [];
    try {
        // Получаем статистику
        const totalForms = await Form_1.default.countDocuments();
        const totalFields = await FormField_1.default.countDocuments();
        const fieldsWithForm = await FormField_1.default.countDocuments({
            formId: { $exists: true },
        });
        const fieldsWithoutForm = await FormField_1.default.countDocuments({
            formId: { $exists: false },
        });
        ...
        const forms = await Form_1.default.find({}, { _id: 1 });
        const formIds = forms.map(form => form._id.toString());
        for (const formId of formIds) {
            const fieldsCount = await FormField_1.default.countDocuments({ formId });
        ...
        const fieldsWithInvalidFormId = await FormField_1.default.find({
        ...
        const sampleField = await FormField_1.default.findOne({ formId: { $exists: true } });
        if (sampleField) {
 ...
 */
async function autoFixDatabaseIssues() {
    const fixed = [];
    const errors = [];
    try {
        // Исправление 1: Привязка полей без formId к первой форме
        const fieldsWithoutForm = await FormField_1.default.find({
            formId: { $exists: false },
        });
        if (fieldsWithoutForm.length > 0) {
            const firstForm = await Form_1.default.findOne();
            if (firstForm) {
                await FormField_1.default.updateMany({ formId: { $exists: false } }, { formId: firstForm._id.toString() });
        ...
        const orphanFields = await FormField_1.default.find({
 ...
 */
async function validateAndFixDatabase(autoFix = false) {
