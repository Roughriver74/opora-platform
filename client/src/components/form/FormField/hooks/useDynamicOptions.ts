...
export const useDynamicOptions = (dynamicSource?: { enabled: boolean; source: string }) => {
  ...
  const loadDynamicOptions = useCallback(async (query: string) => {
    ...
    if (trimmedQuery.length > 0 && trimmedQuery.length < FIELD_CONSTANTS.MIN_SEARCH_LENGTH) {
      return;
    }
    ...
    try {
      ...
      if (selectedOption && !dataOptions.some((opt: FormFieldOption) => opt.value === selectedOption.value)) {
        dataOptions.unshift(selectedOption);
      }
      
      setOptions(dataOptions);
    } catch (error) {
      console.error(`Ошибка при загрузке данных из ${dynamicSource.source}:`, error);
      setOptions([]);
    } finally {
      setLoading(false);
    }
