...
import { createFieldMapping, getDefaultFieldMappings } from '../utils/fieldMapper';
import { convertFieldValue } from '../utils/valueConverter';
import { validateCopyOperation, canCopyBetweenSections } from '../utils/fieldValidator';
...
export const useLinkedFields = ({ sections, values, onValuesChange }: UseLinkedFieldsProps) => {
      ...
      sections.forEach(targetSection => {
        if (sourceSection.title !== targetSection.title) {
          ...
          sourceSection.fields.forEach(sourceField => {
            if (sourceField.linkedFields?.enabled && sourceField.linkedFields.mappings) {
              sourceField.linkedFields.mappings.forEach(mapping => {
                const targetField = targetSection.fields.find(f => f.name === mapping.targetFieldName);
                if (targetField && (mapping.copyDirection === 'from' || mapping.copyDirection === 'both')) {
                  manualMappings.push({
          ...
          // Также проверяем обратные связи (to направление)
          targetSection.fields.forEach(targetField => {
            if (targetField.linkedFields?.enabled && targetField.linkedFields.mappings) {
              targetField.linkedFields.mappings.forEach(mapping => {
                const sourceField = sourceSection.fields.find(f => f.name === mapping.targetFieldName);
                if (sourceField && (mapping.copyDirection === 'to' || mapping.copyDirection === 'both')) {
                  manualMappings.push({
                    sourceField: sourceField.name,
                    targetField: targetField.name,
                    sourceLabel: sourceField.label,
          ...
          automaticMapping.mappings.forEach(autoMapping => {
            const hasManualMapping = manualMappings.some(
              manual => manual.sourceField === autoMapping.sourceField && 
  ...
  const canCopyFields = useCallback((fromSection: string, toSection: string): boolean => {
  ...
  const getAvailableMappings = useCallback((fromSection: string, toSection: string): FieldMapping[] => {
  ...
  const createCopyPreview = useCallback((
    ...
    const changes = mappingsToUse.map(mapping => {
      ...
      return {
        ...
        oldValue,
        newValue: sourceField && targetField 
          ? convertFieldValue(newValue, sourceField.type, targetField.type)
          : newValue,
        isOverwrite: oldValue !== undefined && oldValue !== null && oldValue !== ''
      };
    });
  ...
  const copyFieldsBetweenSections = useCallback((
    ...
    if (mappingsToUse.length === 0) {
      console.warn(`Нет доступных маппингов для копирования: ${fromSection} -> ${toSection}`);
      return false;
    }
    ...
    mappingsToUse.forEach(mapping => {
      const sourceField = sourceSection.fields.find(f => f.name === mapping.sourceField);
      const targetField = targetSection.fields.find(f => f.name === mapping.targetField);
      const sourceValue = values[mapping.sourceField];

      if (sourceField && targetField && sourceValue !== undefined && sourceValue !== null && sourceValue !== '') {
        const convertedValue = convertFieldValue(sourceValue, sourceField.type, targetField.type);
  ...
  const getSourceSectionsFor = useCallback((targetSection: string): string[] => {
    return sections
  ...
  const getTargetSectionsFor = useCallback((sourceSection: string): string[] => {
    return sections
