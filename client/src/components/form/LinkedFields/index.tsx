...
export const LinkedFields: React.FC<LinkedFieldsProps> = ({
  ...
  const handlePreviewRequest = (fromSection: string, toSection: string): CopyPreviewType | null => {
    return linkedFields.createCopyPreview(fromSection, toSection);
  };

  const performCopy = (fromSection: string, toSection: string) => {
    const success = linkedFields.copyFieldsBetweenSections(fromSection, toSection);
  ...
  if (sourceSection) {
    ...
    return (
      <>
        ...
        >
          <DialogTitle>
            Подтверждение копирования полей
          </DialogTitle>
          <DialogContent>
          ...
          <DialogActions>
            <Button onClick={handleCancelCopy}>
              Отмена
            </Button>
            <Button 
    ...
    <Box sx={{ mt: 2 }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {sections.map(section => {
          const targetSections = linkedFields.getTargetSectionsFor(section.title);
