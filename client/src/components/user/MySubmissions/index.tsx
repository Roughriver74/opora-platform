...
const MySubmissions: React.FC = () => {
  ...
  const handleStatusChange = async (submissionId: string, newStatus: string) => {
    try {
      await submissionService.updateStatus(submissionId, newStatus, statusComment);
      setStatusComment('');
  ...
  const getSubmissionFieldValue = (submission: Submission, fieldName: string): string => {
        ...
        <Stack spacing={1} sx={{ mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {submission.bitrixDealId ? (
              <Chip
                icon={
                  submission.bitrixSyncStatus === 'synced' ? <CheckCircleIcon /> :
                  submission.bitrixSyncStatus === 'failed' ? <ErrorIcon /> :
                  <PendingIcon />
                }
                label={
                  submission.bitrixSyncStatus === 'synced' ? 'Синхронизировано' :
                  submission.bitrixSyncStatus === 'failed' ? 'Ошибка синхронизации' :
                  'Ожидает синхронизации'
                }
                color={
                  submission.bitrixSyncStatus === 'synced' ? 'success' :
                  submission.bitrixSyncStatus === 'failed' ? 'error' :
                  'warning'
                }
                size="small"
                variant="outlined"
                          ...
                          {submission.bitrixDealId ? (
                            <Tooltip title={`Сделка ID: ${submission.bitrixDealId}`}>
                              <Chip
                                ...
                                label={
                                  submission.bitrixSyncStatus === 'synced' ? 'Синхр.' :
                                  submission.bitrixSyncStatus === 'failed' ? 'Ошибка' :
                                  'Ожидает'
                                }
                                color={
                                  submission.bitrixSyncStatus === 'synced' ? 'success' :
              ...
              <Card>
                <CardContent>
                  ...
                  <Stack spacing={2}>
                    <Box>
                      <Typography variant="body2" color="text.secondary">
                        <strong>Форма:</strong> {selectedSubmission.formId.title}
                      </Typography>
                    </Box>
                    <Box>
                    ...
                    <Box>
                      <Typography variant="body2" color="text.secondary">
                        <strong>Создано:</strong> {format(new Date(selectedSubmission.createdAt), 'dd.MM.yyyy HH:mm', { locale: ru })}
                      </Typography>
                    </Box>
                    {selectedSubmission.userId && (
                      <Box>
                        <Typography variant="body2" color="text.secondary">
                          <strong>Клиент:</strong> {selectedSubmission.userId.firstName} {selectedSubmission.userId.lastName}
                        </Typography>
                      </Box>
                    )}
                  </Stack>
                  ...
                  <Stack spacing={1}>
                    {Object.entries(selectedSubmission.formData).map(([key, value]) => (
                      <Box key={key} sx={{ 
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ minWidth: { sm: 150 } }}>
                          <strong>{getFieldLabel(key)}:</strong>
                        </Typography>
                        <Typography variant="body2">
                          {Array.isArray(value) ? value.join(', ') : value?.toString() || 'Не указано'}
                        </Typography>
                      </Box>
                    ))}
                  </Stack>
