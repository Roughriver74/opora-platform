...
export const useFieldManagement = (
  ...
  const addNewField = useCallback(() => {
    ...
    const maxOrder = state.fields.length > 0 
  ...
  // Сохранение поля
  const handleFieldSave = useCallback(async (index: number, updatedField: Partial<FormField>) => {
    try {
      ...
      setState(prev => {
        ...
        if (index >= 0 && index < updatedFields.length) {
          ...
          // Если индекс неверный, добавляем в конец
  ...
  // Удаление поля
  const handleFieldDelete = useCallback(async (index: number) => {
