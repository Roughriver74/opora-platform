...
import { CompanyImportResult, CompanyService } from '../../../../services/companyService'
...
export const CompanyImportDialog: React.FC<CompanyImportDialogProps> = ({
	open,
	onClose,
}) => {
	...
	return (
		<Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
			<DialogTitle>Импорт компаний из Excel</DialogTitle>
			...
			<DialogContent>
				...
				{!result && (
					<Box
						...
						onClick={() => document.getElementById('company-import-file-input')?.click()}
					>
						<input
							id='company-import-file-input'
							type='file'
							accept='.xlsx,.xls'
									...
									{result.errors.slice(0, 10).map((err, index) => (
										<ListItem key={index}>
											...
											<ListItemText
												...
												secondary={err.field ? `Поле: ${err.field}` : undefined}
				...
				{!result && (
					<Alert severity='info' sx={{ mt: 2 }}>
						<Typography variant='body2' gutterBottom>
							...
							Колонки: Название*, ИНН, КПП, ОГРН, Тип, Телефон, Email, Адрес и др.
						</Typography>
						<Button
							size='small'
