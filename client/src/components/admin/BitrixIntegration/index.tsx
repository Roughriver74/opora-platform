...
const BitrixIntegration: React.FC = () => {
  ...
  const loadCategories = async () => {
    try {
      ...
      const response = await api.get('/submissions/bitrix/deal-categories');
  ...
  const loadStages = async (categoryId: string = '0') => {
    try {
      ...
      const response = await api.get('/submissions/bitrix/deal-stages', {
        params: { categoryId }
      });
  ...
  const testConnection = async () => {
    try {
      ...
      const response = await api.get('/submissions/bitrix/test-connection');
      ...
      setError('Ошибка подключения к Битрикс24: ' + (err.response?.data?.message || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const testSync = async () => {
    try {
      ...
      const response = await api.post('/submissions/bitrix/test-sync', {
    ...
    <Box>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        ...
        >
          ...
          <Tab icon={<SettingsIcon />} label="Интеграция Битрикс24" {...a11yProps(1)} />
        </Tabs>
      </Box>
            ...
            {loading ? (
              ...
              <>
                ...
                <Typography color="success.main">
                  Подключение активно. Найдено {categories.length} категорий сделок.
                </Typography>
              </>
            ) : (
              <>
                ...
                <Typography color="error.main">
                  Нет подключения к Битрикс24 или отсутствуют категории сделок.
                </Typography>
              </>
            )}
          </Stack>
                          ...
                          <TableCell>
                            {stage.semantic && (
                              <Chip 
                                ...
                                size="small" 
                                color={
                                  stage.semantic === 'S' ? 'success' :
                                  stage.semantic === 'F' ? 'error' :
                                  'default'
                                }
                              />
                            )}
        ...
        <AccordionDetails>
          <Alert severity="info" sx={{ mb: 2 }}>
            <Typography variant="body2">
              Система автоматически сопоставляет наши статусы с подходящими статусами в Битрикс24.
              Алгоритм сначала ищет точные совпадения по ID, затем по названию.
            </Typography>
          </Alert>
